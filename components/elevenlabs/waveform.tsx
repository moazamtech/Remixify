...
export const AudioScrubber = ({
  ...
  // Render custom waveform with progress coloring
  useEffect(() => {
    const canvas = canvasRef.current;
    const container = containerRef.current;
    if (!canvas || !container) return;
    ...
    const resizeObserver = new ResizeObserver(() => {
      ...
      if (ctx) {
        ctx.scale(dpr, dpr);
        renderWaveform(ctx, rect);
      }
    });

    const renderWaveform = (ctx: CanvasRenderingContext2D, rect: DOMRect) => {
      ctx.clearRect(0, 0, rect.width, rect.height);

      const totalBarWidth = barWidth + barGap;
      const barCount = Math.floor(rect.width / totalBarWidth);
      const centerY = rect.height / 2;
      const progressX = localProgress * rect.width;

      for (let i = 0; i < barCount; i++) {
        const dataIndex = Math.floor((i / barCount) * waveformData.length);
        const value = waveformData[dataIndex] || 0;
        const barHeightVal = Math.max(4, value * rect.height * 0.8);
        const x = i * totalBarWidth;
        const y = centerY - barHeightVal / 2;

        // Determine if this bar is before or after the progress point
        const barCenterX = x + barWidth / 2;
        const isPast = barCenterX <= progressX;
        ...
        if (barRadius > 0) {
          ctx.beginPath();
          ctx.roundRect(x, y, barWidth, barHeightVal, barRadius);
          ctx.fill();
        } else {
          ctx.fillRect(x, y, barWidth, barHeightVal);
        }
      }

      // Draw hover indicator line
      if (isHovering && !isDragging) {
        const hoverX = hoverProgress * rect.width;
    ...
    // Re-render when progress or hover state changes
    const ctx = canvas.getContext("2d");
    const rect = container.getBoundingClientRect();
    if (ctx && rect.width > 0) {
      renderWaveform(ctx, rect);
  ...
  return (
    <div
      ...
      {...props}
    >
      {/* Canvas-based waveform with progress coloring */}
      <canvas ref={canvasRef} className="block h-full w-full" />

      {/* Playhead line */}
      <div
        className={cn(
          "pointer-events-none absolute top-0 bottom-0 w-0.5 transition-all duration-75",
          isDragging
            ? "w-1 bg-white shadow-[0_0_10px_rgba(168,85,247,0.8)]"
            : "bg-purple-400",
        )}
      ...
      {showHandle && (
        <div
          className={cn(
            "pointer-events-none absolute top-1/2 -translate-x-1/2 -translate-y-1/2 rounded-full border-2 border-white shadow-lg transition-all duration-150",
            isDragging
              ? "h-6 w-6 bg-purple-500 shadow-[0_0_20px_rgba(168,85,247,0.6)]"
              : isHovering
                ? "h-5 w-5 bg-purple-500 scale-110"
      ...
      {/* Glow effect at playhead */}
      <div
        className={cn(
          "pointer-events-none absolute top-0 bottom-0 w-8 -translate-x-1/2 bg-gradient-to-r from-transparent via-purple-500/20 to-transparent transition-opacity duration-200",
